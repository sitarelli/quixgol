/* script.js - v2.8: MOBILE ZOOM & CAMERA FOLLOW */
...
const cameraLayer = document.getElementById('camera-layer'); // Riferimento al layer camera
...
// --- NUOVA FUNZIONE: GESTIONE CAMERA SU MOBILE ---
function updateCamera() {
    // 1. Controlla se siamo su mobile
    if (window.innerWidth > MOBILE_BREAKPOINT) {
        // Se non siamo su mobile, resetta la trasformazione
        cameraLayer.style.transform = 'translate(0px, 0px) scale(1)';
        return;
    }

    // 2. Calcolo coordinate in pixel del player
    // scaleX converte coordinate griglia (160) a coordinate pixel attuali
    const playerPixelX = (player.x + 0.5) * scaleX;
    const playerPixelY = (player.y + 0.5) * scaleY;

    // 3. Calcolo Centro Viewport
    const viewW = gameWrapper.clientWidth;
    const viewH = gameWrapper.clientHeight;

    // 4. Calcolo traslazione per centrare il player
    // Formula: (Metà Schermo) - (Posizione Player * Zoom)
    let transX = (viewW / 2) - (playerPixelX * MOBILE_ZOOM_LEVEL);
    let transY = (viewH / 2) - (playerPixelY * MOBILE_ZOOM_LEVEL);

    // 5. Clamping: Evitare di vedere lo spazio nero fuori dalla mappa
    // Il limite massimo a sinistra è 0.
    // Il limite massimo a destra è la differenza tra dimensione zoomata e dimensione viewport.
    const maxTransX = 0;
